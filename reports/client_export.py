...
# ── Agency markers (lowercase) — anti-agency belt-and-suspenders filter ──────
# The owner_type="particular" classification is ML-based and not perfect.
# These string markers in the contact_name field are a final defensive layer
# that catches the most common PT real-estate agency brands and roles.
AGENCY_NAME_MARKERS: tuple[str, ...] = (
    # Big franchises (case-insensitive substring match)
    "era ", "era,", "era-", "remax", "re/max", "century 21", "century21",
    "engel", "sotheby", "coldwell", "jll ", "cushman", "savills", "kw ",
    "keller williams",
    # PT-specific brands
    "decisões e soluções", "ideias e casas", "habitat", "habinvest",
    "predial", "comprarcasa", "homing", "lardoce", "century",
    "casashopping", "century 21",
    # Generic role/category markers (very strong agency indicators)
    "imobiliária", "imobiliaria", "imoveis ", "mediação", "mediacao",
    "agência imobi", "agencia imobi",
    "consultor imobi", "consultora imobi", "agente imobi",
    "broker", "real estate", "real-estate",
    "construtora ", "promotor imobi", "promotora imobi",
)


# ── Sheet header definitions (used even when zero rows) ──────────────────────
# When a sheet has no data we still want the proper column headers so the
# xlsx structure matches the client briefing exactly.
SHEET_HEADERS: dict[str, list[str]] = {
    "Básico":       ["Data de entrada", "Origem (fonte)", "Nome", "Contacto", "Link"],
    "FSBO_FRBO":    ["Data de entrada", "Origem (fonte)", "Nome Proprietário",
                     "Contacto", "Imóvel", "Tipologia", "Freguesia", "Concelho",
                     "Valor imóvel", "Link"],
    "Linkedin":     ["Data de entrada", "Origem (fonte)", "Nome", "Contacto",
                     "Email", "Aniversário", "Link"],
    "Standvirtual": ["Data de entrada", "Origem (fonte)", "Nome", "Contacto",
                     "Tipo de produto à venda", "Valor do produto", "Link"],
}


# Descriptive text for the "Objectivo" sheet — verbatim from the client briefing
OBJECTIVO_TEXT: str = (
    "Objectivo do scraping: contacto telefónico de particulares (pessoas) "
    "que tenham colocado o imóvel à venda ou arrendamento e pessoas para "
    "qualificação de vontade/necessidade de venda. "
    "Localização: Concelhos Almada, Seixal e Sesimbra | "
    "Concelhos Lisboa, Oeiras, Cascais e Sintra"
)
...
def _is_individual(lead: Lead) -> bool:
    ...
    Defensive layers (in order):
      1. Phone present
      2. owner_type not in agency/bank/developer/auction
      3. seller_super_flag not raised (camouflaged agency)
      4. contact_name not empty (client requires "Nome")
      5. contact_name doesn't match any agency marker
    """
    ...
    if not name:
        return False           # client wants a real human name, not blanks
    name_lower = name.lower()
    if any(marker in name_lower for marker in AGENCY_NAME_MARKERS):
        return False           # belt-and-suspenders catch (e.g. "Juliano - ERA")
    return True
...
def build_client_xlsx(output_path: Optional[str | Path] = None) -> Path:
    ...
    sheets: dict[str, list[dict]] = {
        ...
        "Objectivo":    [],   # placeholder — rendered specially below
    }
    ...
    for sheet_name, rows in sheets.items():
        ...
        # ── Special-case: "Objectivo" sheet — descriptive text only ─────────
        if sheet_name == "Objectivo":
            ws.append(["Objectivo do scraping"])
            ws["A1"].font = Font(bold=True, color="34D399", size=12, name="Inter")
            ws["A1"].fill = HEADER_FILL
            ws.append([OBJECTIVO_TEXT])
        ...
        # Headers — always write the canonical column list, even if zero rows.
        # This guarantees the xlsx structure matches the client briefing
        # exactly, sheet-by-sheet.
        headers = SHEET_HEADERS.get(sheet_name) or (
