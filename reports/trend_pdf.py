...
_BRAND_PRIMARY = "#a8861a"           # gold deep (calibrated to logo)
_BRAND_GOLD    = "#ddc269"           # gold accent (logo exact)
...
def generate_trend_report(
    ...
    sub_style = ParagraphStyle(
        ...
        textColor=colors.HexColor("#5c5240"),
        spaceAfter=14,
    )
    ...
    if _LOGO_PATH.exists():
        # Logo native ratio is 313:94 (~3.33). Render at 5cm wide ≈ 1.5cm tall.
        story.append(Image(str(_LOGO_PATH), width=5 * cm, height=1.5 * cm))
        story.append(Spacer(1, 0.3 * cm))
    story += [
        Paragraph("Relatório semanal", title_style),
        Paragraph(week_label, sub_style),
    ]
    hero_data = [[
        Paragraph(f"<b>{summary['new_leads_7d']}</b><br/><font size=8 color='#5c5240'>NOVOS LEADS</font>", body_style),
        Paragraph(f"<b>{summary['new_hot_7d']}</b><br/><font size=8 color='#5c5240'>NOVOS HOT</font>", body_style),
        Paragraph(f"<b>{summary['price_drops_7d']}</b><br/><font size=8 color='#5c5240'>QUEDAS DE PREÇO</font>", body_style),
        Paragraph(f"<b>{summary['super_sellers']}</b><br/><font size=8 color='#5c5240'>SUPER-SELLERS</font>", body_style),
        Paragraph(f"<b>{summary['contacted']}</b><br/><font size=8 color='#5c5240'>CONTACTADOS</font>", body_style),
    ]]
    hero_tbl = Table(hero_data, colWidths=[3.5 * cm] * 5)
    hero_tbl.setStyle(TableStyle([
        ("BACKGROUND", (0, 0), (-1, -1), colors.HexColor("#f1f5f9")),
        ("BOX",        (0, 0), (-1, -1), 0.5, colors.HexColor("#d6cdb8")),
    ...
    story += [
        Spacer(1, 0.7 * cm),
        Paragraph(
            f"<font size=8 color='#a89c80'>Gerado por Patabrava em "
...
def _branded_table_style():
    ...
    return TableStyle([
        ("BACKGROUND",  (0, 0), (-1, 0), colors.HexColor("#141008")),
        ("TEXTCOLOR",   (0, 0), (-1, 0), colors.HexColor("#ddc269")),
        ...
        ("ROWBACKGROUNDS", (0, 1), (-1, -1),
            [colors.white, colors.HexColor("#f5efe0")]),
        ("INNERGRID",   (0, 0), (-1, -1), 0.3, colors.HexColor("#e2e8f0")),
        ("BOX",         (0, 0), (-1, -1), 0.5, colors.HexColor("#d6cdb8")),
...
def _default_output_path() -> Path:
    ...
    return settings.data_dir / f"patabrava_trend_{datetime.utcnow().strftime('%Y%m%d')}.pdf"
